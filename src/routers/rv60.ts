...
router_rv60.get("/", async (c: Context) => {
  const books = [];
...
router_rv60.get("/oldTestament/:book", async (c: Context) => {
  try {
...
router_rv60.get("/oldTestament/:book/:chapter", async (c: Context) => {
  try {
...
router_rv60.get("/newTestament/:book", async (c: Context) => {
  try {
    const bookName = c.req.param("book");
...
router_rv60.get("/newTestament/:book/:chapter", async (c: Context) => {
  try {
    const book = c.req.param("book");
...
router_rv60.get("/book/:bookName", async (c: Context) => {
  try {
...
router_rv60.get("/book/:bookName/:chapter", async (c: Context) => {
  try {
    const book = c.req.param("bookName");
    let chapterBook;
    if (isInOldTestament(book)) {
